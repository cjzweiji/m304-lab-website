...
1. Create a Supabase project and run `schema.sql` in the SQL Editor. If the original schema is already installed, run `profile-migration.sql` instead; it adds gender, profile completion, avatar fields and the public `avatars` bucket without recreating any tables.
...
After a member accepts an invitation, the login page asks for a username, gender and an optional JPG/PNG/WebP avatar (maximum 2 MB). Existing members who have not completed a profile will see the same form on the home page; they can reopen it with `编辑资料`.
